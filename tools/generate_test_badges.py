"""Generate shields.io endpoint-badge JSON files for VULCAN test counts.
...
The "tests" total counts every non-skipped test, so it is a strict
superset of the two sub-categories: a test that carries no tier marker is
counted in the total but in neither "unit tests" nor "integration tests".
The three counts therefore need not sum, and "unit tests" plus
"integration tests" can be less than "tests".
