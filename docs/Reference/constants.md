...
|---|---|---|---|
| `kb` | Boltzmann constant | `1.38064852e-16` | erg K⁻¹ |
| `Navo` | Avogadro's number | `6.02214086e23` | mol⁻¹ |
...
!!! note
    The universal gas constant is $R = k_B \times N_\text{avo}$, and the specific gas constant
    for a species with molar mass $M$ is $R_\text{spec} = R / M$.
    `hc` is used to convert the stellar flux to actinic flux (photons cm⁻² s⁻¹ nm⁻¹).