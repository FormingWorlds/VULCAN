...
+ n_i\left(\frac{1}{H_i} + \frac{1+\alpha_T}{T}\frac{\mathrm{d}T}{\mathrm{d}z}\right)\right] \tag{2}$$
...
!!! info "Two forms of Equation 2"
    Eq. 2 given above is the version as published in Tsai et al. (2021) [^tsai2021]. In the code, the term in round brackets is written as $-1/H_0 + 1/H_i + (\alpha_T/T)\,\mathrm{d}T/\mathrm{d}z$, where $H_0 = k_B T/(\bar m g)$ is the
...
The upwind advection appears as the `(vz>0)`/`(vz<0)` masks. Interface quantities ($T_i$, $H_{p,i}$, $\Delta z_i$) are
...
Newton–Raphson iteration for $n_{k+1}$. The Rosenbrock method is *linearly implicit*: it uses
...
$$(I - \gamma \Delta t J)\,g_1 = f(n_k), \qquad
(I - \gamma \Delta t J)\,g_2 = f(n_k + \Delta t\, g_1) - 2 g_1$$
...
The step size adapts to the truncation error $\mathcal{E} = |n_{k+1} - n^*_{k+1}|$, where
$n^*_{k+1} = n_k + \Delta t\, g_1$ is the embedded first-order estimate. In
[`Ros2.step_size`](../Reference/api/op.md#vulcan.op.Ros2.step_size),
 
$$\Delta t_{k+1} = \Delta t_k \cdot \mathrm{clamp}\!\left(0.9\,(\mathrm{rtol}/\mathcal{E})^{1/2},\ r_{\min},\ r_{\max}\right)$$
...
$$\Delta\hat{n} \equiv \max_{i,j} \frac{|n_{i,j,k} - n_{i,j,k'}|}{n_{i,j,k}},
\qquad \Delta t \equiv t_k - t_{k'}$$
 
where $k'$ is the timestep at $f\tau$. Species with mixing ratios below `mtol_conv` or number
...
local vertical-diffusion timescale, $\sim \min_j K_{zz,j}/(0.1\,H_{p,j})^2$. With photochemistry
