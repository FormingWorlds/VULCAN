...
$$\frac{\mathrm{d}n_A}{\mathrm{d}t} = -\frac{D_A\,m_A}{\rho_p\,r_p^{2}}\left(n_A - n_A^\mathrm{sat}\right) n_A \tag{14}$$

where $D_A$ and $m_A$ are the molecular diffusion coefficient and mass of gas $A$, $\rho_p$ and
$r_p$ the particle density and radius, and $n_A^\mathrm{sat}$ the saturation number density.
...
$$v_s = \frac{2\,r_p^{2}\,\rho_p\,g}{9\,\mu} \tag{15}$$
